...
template <typename T>
class PROMETHEUS_CPP_CORE_EXPORT FamilyEnhanced : public CollectableEnhanced {
 public:
  ...
  FamilyEnhanced(const std::string& name, const std::string& help,
  ...
  std::shared_ptr<T> Add(const Labels& labels, Args&&... args) {
    return add(labels, detail::make_shared<T>(args...));
  }

  std::shared_ptr<T> get(const Labels&);

  /// \brief Remove the given dimensional data filtered by labels.
  ///
  /// \param labels labels of a dimension data
  ...
  const Labels& get_constant_labels() const;
  ...
  std::vector<MetricFamily> Collect(bool clear = false) const override;

 private:
  mutable std::unordered_map<Labels, std::shared_ptr<T>, detail::LabelHasher> metrics;

  const std::string name;
  const std::string help;
  const Labels constant_labels;

  ClientMetric collect_metric(const Labels& labels,
                              std::shared_ptr<T> metric) const;
  std::shared_ptr<T> add(const Labels& labels, std::shared_ptr<T> object);
};
