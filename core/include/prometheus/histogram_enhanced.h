...
/// a data race.
class PROMETHEUS_CPP_CORE_EXPORT HistogramEnhanced {
 public:
  using BucketBoundaries = std::vector<double>;

  static const MetricType metric_type{MetricType::HistogramEnhanced};
  ...
  explicit HistogramEnhanced(const BucketBoundaries& buckets);

  /// \copydoc Histogram::Histogram(const BucketBoundaries&)
  explicit HistogramEnhanced(BucketBoundaries&& buckets);
 ...
 private:
  ...
  std::vector<CounterEnhanced> bucket_counts_;
  GaugeEnhanced sum_;
};
...
/// Register(Registry&).
PROMETHEUS_CPP_CORE_EXPORT detail::BuilderEnhanced<HistogramEnhanced>
BuildHistogramEnhanced();
